...
// LOAD THE JSON WITH THE PROMPT DATA
...
loadJsonData().then(jsonData => {
  if (jsonData) {
    ...
    setupButtonEventListeners(jsonData)
  }
});
...
function setupButtonEventListeners(jsonData) {
  const contextBtns = document.querySelectorAll(".context-btn");
...
function generateOutput(jsonData) {
  const promptInput = document.getElementById("prompt-input");
...
function getSelectedOption(buttonType) {
  ...
  if (selectedOption) {
    // Return only the last part of the id after splitting it by the hyphen
    return selectedOption.id.split("-")[1];
  }
  return "";
}
...
copyPromptBadge.addEventListener("click", () => {
  console.log("copyPromptBadge clicked"); // Add this line for debugging
